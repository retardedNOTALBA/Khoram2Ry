...
export function ConfigFree({ lang, t, installed, onAdd }: {
  lang: Lang;
  t: Text;
  installed: Profile[];
  ...
  const load = async () => {
    ...
    try {
      ...
      setError(t("دریافت لیست کانفیگ‌های رایگان انجام نشد. اتصال اینترنت یا آدرس مخزن را بررسی کن.", "Could not load free configurations. Check the internet connection or repository URL."));
    } finally {
      setLoading(false);
    }
  };
  ...
  return <div className="space-y-4">
    ...
    <div className="flex items-center justify-between gap-2">
      <p className="text-[11px] text-white/35">{items.length} {t("کانفیگ قابل دریافت", "available configurations")}{updatedAt ? ` · ${new Date(updatedAt).toLocaleDateString(lang === "fa" ? "fa-IR" : "en-US")}` : ""}</p>
      <Button onClick={() => void load()} disabled={loading} className="h-9 min-h-9 px-3 text-[11px]"><RefreshCw size={13} className={loading ? "animate-spin" : ""} />{t("به‌روزرسانی", "Refresh")}</Button>
    </div>

    {error && <Notice danger>{error}</Notice>}
    {loading && !items.length && <div className="flex items-center justify-center py-14 text-white/40"><Loader2 size={24} className="animate-spin" /></div>}
    {!loading && !error && !items.length && <p className="py-14 text-center text-[12px] text-white/35">{t("فعلاً کانفیگی در مخزن منتشر نشده.", "No configurations are published yet.")}</p>}
